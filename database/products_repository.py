...
class ProductsRepository(DatabaseMethods):
    ...
    def insert(self, item):  # inserts new product into database
        self.cursor.execute(
            "INSERT INTO products(product_name, product_price, product_quantity, product_category, product_id) VALUES(?, ?, ?, ?, ?)",
    ...
    def update(self, id, item):  # updates existing product information
        self.cursor.execute(
            "UPDATE products SET product_name=?, product_price=?, product_quantity=?, product_category=? WHERE product_id=?",
